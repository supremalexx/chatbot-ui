...
export default async function Login({
  ...
  if (session) {
    ...
    if (error || !workspaces || workspaces.length !== 1) {
  ...
  const signIn = async (formData: FormData) => {
    ...
    if (homeWorkspaceError || !workspaces || workspaces.length !== 1) {
      return redirect("/setup")
    }
  ...
  const signUp = async (formData: FormData) => {
    ...
    if ((emailDomainWhitelist.length || emailWhitelist.length) && !domainMatch && !emailMatch) {
